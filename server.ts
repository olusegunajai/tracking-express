...
async function startServer() {
  ...
  app.put("/api/packages/:id", authenticate, async (req, res) => {
    ...
    try {
      ...
      const currentPkg = await db.prepare("SELECT status FROM packages WHERE id = ?").get(req.params.id) as any;
  ...
  app.get("/api/packages/:id/history", authenticate, async (req, res) => {
    const history = await db.prepare("SELECT * FROM package_history WHERE package_id = ? ORDER BY timestamp DESC").all(req.params.id);
    res.json(history);
  });
  ...
  app.post("/api/packages/bulk-delete", authenticate, async (req, res) => {
    const { ids } = req.body;
    if (!Array.isArray(ids)) return res.status(400).json({ error: "ids must be an array" });
    
    const placeholders = ids.map(() => "?").join(",");
    await db.prepare(`DELETE FROM packages WHERE id IN (${placeholders})`).run(...ids);
  ...
  app.post("/api/packages/bulk-status", authenticate, async (req, res) => {
    ...
    const placeholders = ids.map(() => "?").join(",");
    await db.prepare(`UPDATE packages SET status = ? WHERE id IN (${placeholders})`).run(status, ...ids);
  ...
  app.get("/api/content", async (req, res) => {
    const content = await db.prepare("SELECT * FROM content").all();
    res.json(content);
  });
  ...
  app.post("/api/content", authenticate, async (req, res) => {
    ...
    try {
      await db.prepare("INSERT INTO content (section, title, body) VALUES (?, ?, ?)").run(section, title, body);
  ...
  app.put("/api/content/:section", authenticate, async (req, res) => {
    const { title, body } = req.body;
    await db.prepare("UPDATE content SET title = ?, body = ? WHERE section = ?").run(title, body, req.params.section);
    res.json({ message: "Content updated" });
  });

  app.delete("/api/content/:section", authenticate, async (req, res) => {
    await db.prepare("DELETE FROM content WHERE section = ?").run(req.params.section);
    res.json({ message: "Content deleted" });
  });

  // Routes CRUD
  app.get("/api/routes", async (req, res) => {
    const routes = await db.prepare("SELECT * FROM routes").all();
    res.json(routes);
  });

  app.post("/api/routes", authenticate, async (req, res) => {
    const { name, origin, destination, distance, estimated_time } = req.body;
    await db.prepare("INSERT INTO routes (name, origin, destination, distance, estimated_time) VALUES (?, ?, ?, ?, ?)").run(name, origin, destination, distance, estimated_time);
    res.status(201).json({ message: "Route created" });
  });

  app.put("/api/routes/:id", authenticate, async (req, res) => {
    const { name, origin, destination, distance, estimated_time } = req.body;
    await db.prepare("UPDATE routes SET name = ?, origin = ?, destination = ?, distance = ?, estimated_time = ? WHERE id = ?").run(name, origin, destination, distance, estimated_time, req.params.id);
  ...
  app.get("/api/routes/:id/packages", authenticate, async (req, res) => {
    const packages = await db.prepare("SELECT * FROM packages WHERE route_id = ?").all(req.params.id);
    res.json(packages);
  });

  app.get("/api/routes/:id/available-packages", authenticate, async (req, res) => {
    const route = await db.prepare("SELECT origin, destination FROM routes WHERE id = ?").get(req.params.id) as any;
  ...
  app.post("/api/routes/:id/packages/assign", authenticate, async (req, res) => {
    const { packageIds } = req.body;
    if (!Array.isArray(packageIds)) return res.status(400).json({ error: "packageIds must be an array" });
    
    const placeholders = packageIds.map(() => "?").join(",");
    await db.prepare(`UPDATE packages SET route_id = ? WHERE id IN (${placeholders})`).run(req.params.id, ...packageIds);
    res.json({ message: "Packages assigned successfully" });
  });

  app.delete("/api/routes/:id/packages/:packageId", authenticate, async (req, res) => {
    await db.prepare("UPDATE packages SET route_id = NULL WHERE id = ? AND route_id = ?").run(req.params.packageId, req.params.id);
  ...
  app.post("/api/users", authenticate, async (req, res) => {
    ...
    try {
      ...
      await db.prepare("INSERT INTO users (username, password, role) VALUES (?, ?, ?)").run(username, hashedPassword, role);
